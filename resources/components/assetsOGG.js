...
function loadFootballTextures() {
    ...
    return new Promise((resolve) => {
        ...
        textureLoader.load('../models/textures/football_Base_Color.png', texture => {
            footballTextures.baseColor = texture;
            console.log("Check1")
            checkAllAssetsLoaded();
        },
            undefined,  // onProgress callback (can be left as undefined)
            error => {
        ...
        textureLoader.load('../models/textures/football_Mixed_AO.png', texture => {
            footballTextures.mixedAO = texture;
            checkAllAssetsLoaded();
        });

        // Load the football normal color texture
        textureLoader.load('../models/textures/football_Normal_OpenGL.png', texture => {
            footballTextures.normal = texture;
            checkAllAssetsLoaded();
        });

        // Load the football roughness color texture
        textureLoader.load('../models/textures/football_Roughness.png', texture => {
...
export { loadFootballTextures };  // Exporting the loaded textures for use in other components